...
export function DoctorViewScreen() {
  ...
  const playbackKey = `${activeProjection?.timestamp ?? 0}|${replay}|${sequenceKey}`;
  const index = progress.key === playbackKey ? Math.min(progress.index, Math.max(0, matched.length - 1)) : 0;
